...
function cartesian_nonlinear_laplacian(expr, II::CartesianIndex, derivweights, s::DiscreteSpace, bs, depvars, x, u)
    ...
    map_ivs_to_interpolated(stencil, weights) = safe_vcat([x => dot(weights, getindex.((s.grid[x],), getindex.(interface_wrap(stencil), (j,))))], [s.ivs[k] => s.grid[s.ivs[k]][II[k]] for k in setdiff(1:N, [j])])
